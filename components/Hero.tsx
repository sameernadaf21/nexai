      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8 pt-24 pb-12 w-full flex flex-col lg:flex-row items-center gap-12 lg:gap-0">
        ...
        <div className="w-full lg:w-2/3 space-y-8">
          ...
          <div className="inline-flex items-center gap-2 mb-2 bg-[#009B8F]/10 px-3 py-1 rounded border border-[#009B8F]/30">
            ...
            <span className="text-xs font-bold tracking-widest text-[#009B8F] uppercase">Premium Global Education Consultants</span>
          </div>

          <h1 className="text-5xl md:text-6xl lg:text-7xl leading-tight font-serif font-light tracking-tight">
            Nex Overseas <br />
            <span className="text-[#009B8F] font-normal italic text-3xl md:text-4xl lg:text-5xl block mt-4">
              "From Preparation to Arrival We Make Your Global Journey Possible"
            </span>
          </h1>

          <p className="text-slate-400 font-light text-lg md:text-xl max-w-xl leading-relaxed uppercase tracking-wide">
            Admissions | Coaching | Visa | Travel & Transport | Advisory
          </p>
          ...
          <div className="flex flex-wrap gap-6 pt-8 text-xs font-bold tracking-widest uppercase text-slate-500">
            ...
            <div className="flex items-center gap-2">
              <CheckCircle2 size={16} className="text-[#009B8F]" /> Experts Advice
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#00151a]/80 backdrop-blur-md border border-[#009B8F]/30 p-8 rounded-sm shadow-2xl relative transform rotate-1 hover:rotate-0 transition-transform duration-500 group">
            ...
            <ul className="space-y-4">
              {[
                ...
                '2. Pre-Arrival Coaching',
                '3. Travel & Transport',
                '4. Immigration Advisory',
                '5. Skill Preparation'
              ].map((item) => (
                <li key={item} className="flex justify-between items-center border-b border-slate-700/50 pb-3 group/item cursor-pointer">
                  <span className="text-slate-300 font-sans text-sm group-hover/item:text-white transition-colors">{item}</span>
                </li>
              ))}
            </ul>
